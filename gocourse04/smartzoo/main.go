// При розробці системи «Розумний зоопарк» тех лід ще не вирішив, яку базу даних використовувати та для реалізації
...
// для перебування тварини та технічне приміщення, де можуть зберігатися приладдя для роботи з тваринами. На кожній
...
// Area по типу (копитні, пернаті, примати)
type Area struct {
	Name    string
	Sectors map[string]Sector
}

// Sector для виду тварин. У кожному загоні є технічне приміщення (наприклад годування)
type Sector struct {
	ID      int
	Animals []Animal
}
...
// MoveAnimal - move the animal to another sector
...
// Clean the animal. ID 6 = Bathroom
// the animal moves to the Bathroom sector and returns to the sector from which it came
...
// FeedAnimals - the animals go out to eat - Sector: Dining room
